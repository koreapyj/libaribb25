...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 ...
 ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static void release_b_cas_cobalt(void *bcas);
static int init_b_cas_cobalt(void *bcas);
static int get_init_status_b_cas_cobalt(void *bcas, B_CAS_INIT_STATUS *stat);
static int get_id_b_cas_cobalt(void *bcas, B_CAS_ID *dst);
static int get_pwr_on_ctrl_b_cas_cobalt(void *bcas, B_CAS_PWR_ON_CTRL_INFO *dst);
static int proc_ecm_b_cas_cobalt(void *bcas, B_CAS_ECM_RESULT *dst, uint8_t *src, int len);
static int proc_emm_b_cas_cobalt(void *bcas, uint8_t *src, int len);

/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 global function implementation
 ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
extern "C" B_CAS_CARD *create_b_cas_cobalt()
{
	int n;
	...
	r->release = release_b_cas_cobalt;
	r->init = init_b_cas_cobalt;
	r->get_init_status = get_init_status_b_cas_cobalt;
	r->get_id = get_id_b_cas_cobalt;
	r->get_pwr_on_ctrl = get_pwr_on_ctrl_b_cas_cobalt;
	r->proc_ecm = proc_ecm_b_cas_cobalt;
	r->proc_emm = proc_emm_b_cas_cobalt;
	...
	sys.cardVersion = 2;
	sys.logMode = 1;
	sys.clModeEnable = false;
	sys.CARD_IMAGE_FILE_NAME = std::string("cobaltcas.bin");
	sys.LOG_FILE_NAME = "cobaltcas.log";
	return r;
}
...
static int get_id_b_cas_cobalt(void *bcas, B_CAS_ID *dst)
{
	...
	ret = prv->card->processCmd32(prv->sbuf, slen, prv->rbuf, &rlen);
...
static int get_pwr_on_ctrl_b_cas_cobalt(void *bcas, B_CAS_PWR_ON_CTRL_INFO *dst)
{
	...
	ret = prv->card->processCmd80(prv->sbuf, slen, prv->rbuf, &rlen);
...
static int proc_ecm_b_cas_cobalt(void *bcas, B_CAS_ECM_RESULT *dst, uint8_t *src, int len)
{
	...
	retry_count = 0;
	ret = prv->card->processCmd34(prv->sbuf, slen, prv->rbuf, &rlen);
...
static int proc_emm_b_cas_cobalt(void *bcas, uint8_t *src, int len)
{
	...
	retry_count = 0;
	ret = prv->card->processCmd36(prv->sbuf, slen, prv->rbuf, &rlen);
